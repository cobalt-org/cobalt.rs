...
fn main() {
    ...
    match command {
        ...
        "clean" => {
            ...
            let destdir = std::fs::canonicalize(PathBuf::from(&config.dest)).unwrap_or(PathBuf::new());
            if cwd == destdir {
                warn!("Destination directory is same as current directory. The current directory will be deleted.")
            }
